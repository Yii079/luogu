...
int main()
{
    ...
    {
        string strPos;
        cin >> people[i].name >> strPos >> people[i].value >> people[i].level;
        people[i].pos = posMap.at(strPos);
        people[i].order = i;
    }
    sort(people + 3, people + n, 
    ...
    for (int i = 3; i < n; i++)
    {
        if (i <= 4) people[i].pos = HuFa;
        else if (i <= 8) people[i].pos = ZhangLao;
        else if (i <= 15) people[i].pos = TangZhu;
        else if (i <= 40) people[i].pos = JingYing;
        else people[i].pos = BangZhong;
    }
    sort(people + 3, people + n,
        [](const Person& p1, const Person& p2)
        {
    ...
    for (int i = 0; i < n; i++)
    {
        cout << people[i].name << ' ' << posName[people[i].pos] << ' ' << people[i].level << endl;
    }
    return 0;
}
